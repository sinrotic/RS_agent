...
from rs_core.workflow.hybrid_demo import _feedback_constraints_from_config, recommend_for_user
...
class OnlinePool500Recommender:
    ...
    def recommend(
        ...
        *,
        ...
        feedback_constraints = _feedback_constraints_from_config(self.env.config, feedback_text)
        ...
        display = validate_public_display_payload({
    ...
    def recall(self, request: dict[str, Any]) -> dict[str, Any]:
        """Public online recall boundary backed by the same runtime as agent tools."""

        user_sequence = dict(request.get("user_sequence") or {})
        if request.get("user_id") is not None:
            user_sequence["user_id"] = request["user_id"]
        result = self.tool_retrieve_candidates(
            user_sequence,
            prior_turn_items=set(request.get("prior_turn_items") or []),
            candidate_pool_size=request.get("candidate_pool_size"),
        )
...
def _two_tower_source_readiness_status(source_config: dict[str, Any], manifest_path: Path) -> dict[str, Any]:
    ...
    return {"available": manifest_exists, "status": "configured" if manifest_exists else "missing_manifest", "backend": "local_vector"}
