...
def build_candidate_store_from_env() -> CandidateStore:
    ...
    if backend in {"", "mysql"}:
        from rs_core.online.recall.candidate_store.mysql import build_mysql_candidate_store_from_env

        return build_mysql_candidate_store_from_env()
    if backend == "noop":
        return NoopCandidateStore()
    if backend in {"cassandra", "scylla", "cql"}:
        from rs_core.online.recall.candidate_store.cassandra import build_cassandra_candidate_store_from_env
