...
POOL500_DIAGNOSTIC_EXTRA_SOURCES = {
...
def adapt_pool500_rows_to_candidates(
    ...
    *,
    ...
    for row_index, row in enumerate(rows, start=1):
        row_blockers = _validate_required_fields(row, row_index, allow_score_fallback=allow_score_fallback)
        if row_blockers:
        ...
        score, score_fallback_reason = _row_score(row, source=source, allow_score_fallback=allow_score_fallback)
        if score is None:
            blockers.append(_blocker("POOL500_NON_FINITE_SCORE", {"row_index": row_index, "user_id": user_id, "item_id": item_id, "source": source, "score": row.get("score")}))
            continue
        if score_fallback_reason:
            diagnostics.append(_diagnostic("POOL500_SCORE_FALLBACK_USED", {"row_index": row_index, "user_id": user_id, "item_id": item_id, "source": source, "reason": score_fallback_reason}))
...
def adapt_pool500_jsonl_to_candidates(
    ...
    *,
    ...
    return adapt_pool500_rows_to_candidates(
        iter_jsonl(path),
        candidate_pool_limit=candidate_pool_limit,
        extra_allowed_sources=extra_allowed_sources,
        allow_score_fallback=allow_score_fallback,
    )


def _validate_required_fields(row: dict[str, Any], row_index: int, *, allow_score_fallback: bool = False) -> list[dict[str, Any]]:
    required_fields = REQUIRED_ROW_FIELDS - ({"score"} if allow_score_fallback else set())
...
def _row_score(row: dict[str, Any], *, source: str, allow_score_fallback: bool) -> tuple[float | None, str | None]:
    ...
    if not isinstance(source_scores, dict) and isinstance(row.get("metadata"), dict):
        source_scores = row["metadata"].get("source_scores")
    if isinstance(source_scores, dict):
        source_score = _coerce_score(source_scores.get(source))
        if source_score is not None:
            return source_score, "source_scores"
        finite_scores = [_coerce_score(value) for value in source_scores.values()]
        finite_scores = [value for value in finite_scores if value is not None]
        if finite_scores:
            return max(finite_scores), "source_scores_max"
    rank = _coerce_rank(row.get("rank"))
    if rank is not None:
        return round(1.0 / float(rank), 10), "rank_inverse"
    return None, None
