...
from rs_core.recsys.candidate_merge import merge_candidates
from rs_core.recsys.online_retrieval.config import clamp_int, online_retrieval_config, provider_configs
from rs_core.recsys.online_retrieval.models import OrchestratorResult, ProviderResult, RetrievalRequest
from rs_core.recsys.online_retrieval.provider import CandidateProvider
...
def _build_provider(
    ...
    *,
    ...
    if name == "two_tower_qdrant":
        from rs_core.recsys.online_retrieval.providers.qdrant_two_tower import QdrantTwoTowerProvider

        return QdrantTwoTowerProvider.from_config(provider_config, config_path=config_path)
    if name in {"postgres_item_neighbors", "candidate_store_itemcf_strong", "candidate_store_itemcf_weak", "candidate_store_co_visit_repair"}:
        ...
        return PostgresItemNeighborsProvider.from_config(config_with_source, provider_name=name)
    if name in {"postgres_usercf", "candidate_store_usercf"}:
        ...
        return PostgresUserCFProvider.from_config(config_with_source, provider_name=name)
    if name in {"postgres_popular", "candidate_store_popular"}:
        from rs_core.recsys.online_retrieval.providers.postgres_popular import PostgresPopularProvider

        return PostgresPopularProvider.from_config(provider_config, provider_name=name)
    if name in {"postgres_category", "candidate_store_category"}:
        from rs_core.recsys.online_retrieval.providers.postgres_category import PostgresCategoryProvider

        return PostgresCategoryProvider.from_config(provider_config, provider_name=name)
