...
@dataclass
class PostgresCategoryProvider:
    config: dict[str, Any]
    store: CandidateStore
    enabled: bool = True
    name: str = "postgres_category"
    role: str = "online_category_recall"
    source_name: str = "category"

    @classmethod
    def from_config(cls, provider_config: dict[str, Any], *, provider_name: str | None = None) -> "PostgresCategoryProvider":
        config = dict(provider_config)
        return cls(
            config=config,
            store=build_candidate_store_from_env(),
            enabled=provider_enabled(provider_config),
            name=provider_name or str(config.get("name", "postgres_category")),
