...
from rs_core.recsys.vectorstores.milvus_contracts import MILVUS_RAG_CHUNK_SCHEMA_VERSION, MILVUS_TWO_TOWER_ITEM_SCHEMA_VERSION

SEMANTIC_VECTOR_ITEM_SCHEMA_VERSION = "semantic_vector_item_v1"


def stable_vector_point_id(*parts: Any) -> str:
    ...
    return str(uuid5(NAMESPACE_URL, f"rs-agent:vector:{raw}"))
...
def two_tower_item_payload(
    *,
    ...
    index_build_id: str | None = None,
    schema_version: str = MILVUS_TWO_TOWER_ITEM_SCHEMA_VERSION,
    metadata: dict[str, Any] | None = None,
) -> dict[str, Any]:
...
def semantic_vector_item_payload(
    *,
    ...
    embedding_model_name: str | None = None,
    schema_version: str = SEMANTIC_VECTOR_ITEM_SCHEMA_VERSION,
    metadata: dict[str, Any] | None = None,
) -> dict[str, Any]:
    payload = {key: value for key, value in dict(metadata or {}).items() if key != "embedding"}
    payload.update(
