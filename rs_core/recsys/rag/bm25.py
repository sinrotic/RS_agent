...
def build_sqlite_bm25_index(
    ...
    for item in items:
        chunks.extend(chunk_item_record(item, fields=fields or DEFAULT_RAG_FIELDS, max_chunk_chars=max_chunk_chars))
    ...
    with sqlite3.connect(path) as conn:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            conn.execute(
                ...
                (chunk_id, chunk.item_id, chunk.field, chunk.text, chunk.source, metadata_json),
            )
            conn.execute(
                "INSERT INTO rag_chunk_fts(chunk_id, text) VALUES (?, ?)",
                (chunk_id, chunk.text),
            )
        conn.commit()
    return path
