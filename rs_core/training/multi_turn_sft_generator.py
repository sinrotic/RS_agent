        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "Assume the customer does not know or care that the system used history, retrieval, ranking, or a display slate; never mention those mechanics. "
                    "Recommend 2-3 products with short product names and concrete use-case reasons, not long raw titles. "
                    "Say why the product itself helps with the customer's current need; do not say it matches past purchases, browsing history, scores, or candidates. "
...
def _is_grounded_explanation_response(text: str, diagnostics: dict[str, Any], history: list[dict[str, Any]]) -> bool:
    ...
    if not any(turn.get("display_item_ids") for turn in history if isinstance(turn, dict)):
        return False
    return bool(re.search(r"最近一次推荐列表里推荐|我推荐.+主要是因为|主要因为|因为它|商品信息显示", text))
...
def _sanitize_dialogue_only_assistant_message(text: str) -> str:
    if re.search(r"\bB[A-Z0-9]{9}\b|parent_asin|item_id|商品ID|商品编号|\b推荐\b|最近一次推荐列表里推荐", text, re.IGNORECASE):
        return "我还需要更多偏好信息来确认需求，暂不提供具体选项。"
    sanitized = re.sub(r"展示评分为\s*[0-9.]+[。.]?", "", text).strip()
    sanitized = re.sub(r";\s*。", "。", sanitized).strip()
    return sanitized or text
    ...
    return [
        {
            ...
            "content": (
                ...
                "For initial_action, return JSON {\"action_type\": \"chat\", \"message\": string} with a natural first request; do not recite category/keyword lists. "
