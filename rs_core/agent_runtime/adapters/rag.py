...
from rs_core.agent_runtime.core.events import TraceEvent
...
from rs_core.rsagent.schema import AgentTurn
...
_SENSITIVE_TEXT_PATTERN = re.compile(r"(?i)(source|score|manifest|path|provenance|oracle|holdout|label|target|future|test|retriever|bm25|sqlite_bm25|qdrant|hybrid|vector|source_fields)\b|[/\\][^\s]+")
_QUERY_REWRITE_FORBIDDEN_PATTERN = re.compile(
    r"(?i)(retrieve_candidates|rank_candidates|tool call|internal tool|diagnostic|trace|source[_ -]?score|source[:=]|score[:=]|sqlite_bm25|bm25|qdrant|oracle|label|manifest|holdout|future|candidate[_ -]?id|item[_ -]?id|[/\\][^\s]+)"
)
