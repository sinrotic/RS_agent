...
def build_recall_views(
    *,
    input_dir: Path,
    output_dir: Path,
    recent_window_ratio: float = DEFAULT_RECENT_WINDOW_RATIO,
    max_items_per_user_for_itemcf: int = DEFAULT_MAX_ITEMS_PER_USER_FOR_ITEMCF,
    category_top_k: int = DEFAULT_CATEGORY_TOP_K,
    item_graph_window: int = DEFAULT_ITEM_GRAPH_WINDOW,
    item_graph_top_k: int = DEFAULT_ITEM_GRAPH_TOP_K,
    item_graph_min_score: float = DEFAULT_ITEM_GRAPH_MIN_SCORE,
    item_graph_strong_multiplier: float = DEFAULT_ITEM_GRAPH_STRONG_MULTIPLIER,
    lightweight_full_safe: bool = False,
    lightweight_max_output_bytes: int = DEFAULT_LIGHTWEIGHT_MAX_OUTPUT_BYTES,
    lightweight_min_free_bytes: int = DEFAULT_LIGHTWEIGHT_MIN_FREE_BYTES,
    semantic_inverted_top_k: int = DEFAULT_SEMANTIC_INVERTED_TOP_K,
) -> tuple[dict[str, Any], dict[str, Any]]:
    ensure_args(
        recent_window_ratio,
        max_items_per_user_for_itemcf,
        category_top_k,
        item_graph_window,
        item_graph_top_k,
        item_graph_min_score,
        item_graph_strong_multiplier,
    )

    if lightweight_full_safe:
        return build_lightweight_full_safe_views(
            input_dir=input_dir,
            output_dir=output_dir,
            recent_window_ratio=recent_window_ratio,
            category_top_k=category_top_k,
            min_free_bytes=lightweight_min_free_bytes,
            max_output_bytes=lightweight_max_output_bytes,
            semantic_inverted_top_k=semantic_inverted_top_k,
        )
    ...
    stats_payload = {
        ...
        "config": {
            "recent_window_ratio": recent_window_ratio,
            "max_items_per_user_for_itemcf": max_items_per_user_for_itemcf,
            "category_top_k": category_top_k,
            "item_graph_window": item_graph_window,
            "item_graph_top_k": item_graph_top_k,
            "item_graph_min_score": item_graph_min_score,
            "item_graph_strong_multiplier": item_graph_strong_multiplier,
        },
        "popular_recall": popularity_stats,
    ...
    stats_payload["stats_path"] = str(stats_path)
    return manifest_payload, stats_payload
