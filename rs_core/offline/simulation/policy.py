...
from rs_core.offline.simulation.model_client import SimulationModelUnavailableError
...
class RolePolicy:
    def next_action(self, role: SimulatedCustomerRole, state: RoleState, display_response: dict[str, Any]) -> RoleAction:
        ...
        if best_score > 2.0:
            ...
            return RoleAction.accept(best_id, "This fits what I was trying to solve.")

        if best_score >= 1.0 and not _assistant_explains_display_items(assistant_message, items):
    ...
    return [
        {
            ...
            "content": (
                ...
                "For chat actions, write a natural customer message about your need, question, or constraint; express the underlying preference in your own words instead of reciting category or keyword lists. "
                "Judge the interaction by semantic progress, not fixed trigger phrases: decide whether the assistant understood your need, resolved uncertainty, or adjusted after feedback. "
...
def _assistant_explains_display_items(message: str, items: list[dict[str, Any]]) -> bool:
    ...
    for item in items[:3]:
        ...
        if (item_id and item_id in normalized) or (title_token and title_token in normalized):
            mentioned += 1
        evidence_overlap += _item_evidence_overlap(normalized, item)
    return mentioned >= 1 and evidence_overlap >= 2


def _item_evidence_overlap(normalized_message: str, item: dict[str, Any]) -> int:
    evidence_text = " ".join(
        str(value or "")
        for value in [
            item.get("title"),
    ...
    tokens = {
        token
        for token in re.findall(r"[a-z0-9_]+", evidence_text)
        if len(token) >= 4 and token in normalized_message
    }
    chinese_spans = {
        span
        for span in re.findall(r"[一-鿿]{2,}", evidence_text)
        if span in normalized_message
    }
    return len(tokens) + len(chinese_spans)
