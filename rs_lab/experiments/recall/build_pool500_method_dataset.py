    ...
    *,
    ...
    if blocker:
        manifest = _manifest(
            ...
            item_bucket_policy=_item_bucket_policy(source_method),
            row_count=0,
            user_count=0,
            item_count=0,
...
def _eligible_items(path: Path, item_user_counts: dict[str, int], *, max_item_user_freq: int) -> tuple[set[str], Counter[str]]:
    ...
    *,
    ...
    for sequence in iter_jsonl(path):
        ...
        if len(contributing_users) >= max_output_users:
            ...
            continue
        positives = [str(item_id) for item_id in sequence.get("recent_positive_item_sequence") or [] if item_id]
        filtered_all = _unique_in_order(item_id for item_id in positives if item_id in eligible_items)
        filtered = filtered_all[:max_items_per_user]
        if len(filtered_all) > len(filtered):
            dropped["items_over_user_cap"] += len(filtered_all) - len(filtered)
        if len(filtered) < 2:
    ...
    for (item_i, item_j), support in supported_pairs:
        ...
        for src_item_id, dst_item_id in ((item_i, item_j), (item_j, item_i)):
    ...
    unique_pair_count = len(supported_pairs)
    edge_count = unique_pair_count * 2
    item_ids = {item_id for row in rows for item_id in (row["item_i"], row["item_j"])}
    weighted_cooc_sum_before_topk = round(sum(pair_weighted_cooc[pair] for pair, _support in supported_pairs) * 2, 6)
    ...
    counts = {
        ...
        "feature_summary": {
            ...
            "directed_edge_count_before_topk": edge_count,
            "directed_edge_count_after_topk": len(rows),
            "weighted_cooc_sum_before_topk": weighted_cooc_sum_before_topk,
            "weighted_cooc_sum_after_topk": weighted_cooc_sum_after_topk,
...
def _item_bucket_policy(source_method: str) -> str:
    if source_method == "swing_method_dataset":
