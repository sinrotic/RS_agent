...
def _apply_deepfm_model_score(candidate: MergedCandidate, model: dict[str, Any] | None, config: dict) -> tuple[float, list[dict[str, Any]]]:
    ...
    governance = _deepfm_governance_gate(policy, report, requested_delta)
...
def _deepfm_governance_gate(policy: dict[str, Any], report: dict[str, Any] | None, requested_delta: float) -> dict[str, Any]:
    ...
    if _is_deepfm_shadow_policy(policy):
        delta_allowed = policy_replacement is True and policy_effect is True
    else:
        delta_allowed = policy_replacement is not False and policy_effect is not False
    if report:
