...
from rs_core.offline.simulation.schema import RoleAction, RoleState, SimulatedCustomerRole
from scripts.training.judge_sft_samples import _safe_project_path
from rs_core.offline.training.multi_turn_sft_generator import (
...
from rs_core.offline.training.sft_judge import SFT_JUDGE_SCHEMA_VERSION, judge_sft_sample, judge_sft_samples
...
def test_simulated_user_prompt_uses_private_context_without_hidden_catalog() -> None:
    from rs_core.offline.simulation.schema import RoleState, SimulatedCustomerRole
...
def test_deterministic_composer_writes_item_specific_reasons() -> None:
    ...
    message, metadata = composer.compose(
        ...
        display={
            ...
            "items": [
                ...
                {"parent_asin": "B000000003", "title": "Adjustable Monitor Stand - Home Office Desk Riser", "category": "Office Products"},
            ],
        },
        history=[],
    )
    ...
    assert "从名称看" not in message
    assert "名称比较明确" not in message
    assert "organized" in message or "实用" in message or "场景" in message
