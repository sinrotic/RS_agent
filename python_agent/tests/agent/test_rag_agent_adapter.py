...
from rs_core.agent.runtime_core import AgentRunRequest, AgentRunResult
from rs_core.agent.adapters.rag import (
...
def test_rag_agent_redacts_bare_retriever_identity_text() -> None:
    turn = _turn(
        rag_context={
            ...
            "candidate_item_ids": ["i1"],
            "evidence": [
                {"item_id": "i1", "field": "features", "text": "sqlite_bm25 retrieved this candidate via milvus vector", "metadata": {}},
            ],
        }
    )
    ...
    for blocked in ("sqlite_bm25", "milvus", "vector"):
