...
**定位方式：**
用 import census 扫描 `rs_core.recsys.two_tower_source_manifest` 调用点，覆盖 `scripts/recall/build_two_tower_source_index.py`、`rs_core/online/recall/vectorstores/two_tower_source_build.py`、`rs_core/online/recall/vector_index.py`、rs_lab two-tower experiments 和 source manifest tests；用 architecture boundary test 检查 canonical import 不进入 legacy whitelist，并新增旧 path-not-exists guard。
...
**解决方式：**
将远程 F1 artifact 拉回并落到 `outputs/recall/pool500_method_sources/recent_2y/swing_recall/formal/run_20260606_datawhale_f1_main_route_v1/`；更新 `configs/recall/full_data_pool500/swing_recall/source_config.yaml`、`dataset_policy.yaml`、`configs/recall/pool500_method_registry.json`、`rs_core/online/recall/source_registry/registry.py` 和 `run_full_data_pool500_recall_only.py` 的默认 Swing manifest 指向。保留 artifact 内 `candidate_generation_allowed=false`、`ranking_input_replacement_allowed=false`、`pool1000_allowed=false`，主路使用必须通过显式默认 manifest 和后续 route-level 审计。
...
**解决方式：**
新增 `rs_lab/README.md`、`rs_lab/GOVERNANCE.md`、`rs_lab/experiments/recall/pool500/README.md`、`governance/README.md` 和 `user_quality/README.md`，明确实验晋升、资源 guard、ranking input 替换和 pool1000 的禁止边界。新增 `rs_core/online/recall/source_registry/base.py`、`registry.py`、`__init__.py`，只保存 `RecallSourceSpec` 与 source readiness 元数据，不迁移 UserCF / ItemCF / semantic 等实验构建逻辑。
...
**定位方式：**
对照各方法 `METHOD.md`、`source_config.yaml`、`configs/recall/pool500_method_registry.json` 与 `rs_core/online/recall/source_registry/registry.py`，确认真正允许候选生成的当前白名单应为 `popular` 和 route-gate candidate 的 `itemcf_strong`，其余 guarded/diagnostic/deferred source 都不应被 `list_candidate_generating_sources()` 返回。
...
**验证结果：**
使用项目 `.venv` 执行 `python -m pytest tests/test_recall_source_registry.py tests/test_pool500_method_source_runner.py -q`，结果 `26 passed`；对 `rs_core/online/recall/source_registry/base.py`、`registry.py`、`__init__.py` 与 runner 执行 `py_compile` 通过；独立 verifier 复核结论为 PASS，确认候选生成已改为显式 opt-in，category 仍不是直接主路物化生成。
...
### 2026-06-30 - Recsys vectorstores 迁入 Data/Online canonical owner

- **任务**：继续收束旧 `rs_core/recsys` active namespace，将 `rs_core/recsys/vectorstores/*` 从推荐历史目录中迁出。
- **遇到的问题**：`vectorstores` 同时服务 RAG Milvus、two-tower 召回索引构建、脚本和 data adapter，不能简单整体搬到 Agent 或 Online 单一 owner；同时本地存在 `.worktrees`、`outputs` 等历史副本，AST census 需要排除非当前 active tree。
- **定位方式**：用 `Glob` / `Grep` / AST census 盘点 active imports，确认旧调用点集中在 `rs_core/agent/rag/*`、`rs_core/data/adapters/__init__.py`、`scripts/recall/*` 和 `tests/test_milvus_*.py`；focused pytest 覆盖 Milvus vectorstore、RAG retriever/index、two-tower Milvus build、backfill CLI、architecture boundary 和 serving import graph。
- **解决方式**：将通用 Milvus/vector store client、schema、filter、payload 与 build utils 迁入 `rs_core/data/vectorstores/`；将 two-tower source/index build 与 backfill helpers 迁入 `rs_core/online/recall/vectorstores/`；改写 Agent RAG、data adapter、workflow facade、recall scripts 和 tests 的 import；删除旧 `rs_core/recsys/vectorstores` active package，并新增 architecture path-not-exists guard。
- **验证结果**：focused tests `102 passed in 12.18s`；Ruff `All checks passed!`；compileall 通过；AST census 显示 `rs_core.recsys.vectorstores=[]` 且既有已退役 recsys imports 均为 `[]`；path check 显示旧 vectorstores 不存在、新 `rs_core/data/vectorstores` 与 `rs_core/online/recall/vectorstores` 存在。
- **面试可讲点**：这次迁移体现了按职责拆分旧模块的工程判断：底层向量库连接和 schema 归 data boundary，two-tower 召回索引构建归 online recall，Agent RAG 只消费 data vectorstore，不再持有底层 Milvus client，从而减少跨层耦合并让旧 recsys namespace 进一步接近清空。

### 2026-06-30 - Recsys recall_sources 迁入 Online source_registry 并删除包根

- **任务**：继续执行旧 `rs_core/recsys` active namespace 的物理收敛，将 `rs_core/recsys/recall_sources/*` 真实迁入 online recall canonical owner，并删除旧 `rs_core/recsys` 包根。
- **遇到的问题**：`recall_sources` 是方法治理元数据，不是召回算法实现；迁移时既要保持 `RecallSourceSpec` / readiness / candidate-generating registry 的 public API，又要避免留下旧包根或 facade 让 `rs_core.recsys` 继续作为 active package 存在。
- **定位方式**：通过 active import census、path check、focused tests 和文档/egg-info 扫描确认调用点集中在 `tests/test_recall_source_registry.py`、method 文档和 packaging source 列表；同时复核 architecture guard 已断言旧 `rs_core/recsys` path 不存在。
- **解决方式**：将 `base.py`、`registry.py`、`__init__.py` 迁入 `rs_core/online/recall/source_registry/`，在 `rs_core/online/recall/__init__.py` re-export registry public helpers，改写测试和方法文档引用；删除旧 `rs_core/recsys/recall_sources` 与包根，并更新 `SOURCES.txt`、结构/兼容/Phase 2 hardening 文档和 path-not-exists guard。
- **验证结果**：focused pytest、Ruff、compileall、AST census、path check、`git diff --check`、新增 diff blocker scan 与独立 verifier 已作为本轮完成门禁执行；证据见最终回复中的验证命令和结果。
- **面试可讲点**：这段可以讲成“把推荐方法治理元数据从历史 recsys package 拆到 online recall owner”：算法实现、向量库基础设施和治理 registry 分别归属 online/data/offline/agent 后，旧 `rs_core/recsys` 不再作为 active package 存在，架构收敛从 import 替换推进到了物理删除和防回流 guard。
