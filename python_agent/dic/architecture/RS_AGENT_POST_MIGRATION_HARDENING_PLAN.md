...
- [x] 将 `rs_core/recsys/vectorstores` 中通用 Milvus/vector store client、schema、filter、payload 与 build utils 迁入 `rs_core/data/vectorstores`，旧 active package 已删除并由 architecture path-not-exists guard 防恢复。
...
- [x] 将 `rs_core/recsys/vectorstores` 中 two-tower source/index build 与 backfill helpers 迁入 `rs_core/online/recall/vectorstores`，旧 active package 已删除并由 architecture path-not-exists guard 防恢复。
- [x] 将 `rs_core/recsys/recall_sources` 的 `RecallSourceSpec` readiness registry 迁入 `rs_core/online/recall/source_registry`，旧 `rs_core/recsys` active 包根已物理删除并由 architecture path-not-exists guard 防恢复。
