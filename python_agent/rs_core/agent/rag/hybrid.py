...
from rs_core.agent.rag.retriever import CandidateEvidenceRetriever

from rs_core.agent.rag.bm25 import SQLiteBM25CandidateRetriever
from rs_core.agent.rag.corpus import RAG_COMPACT_DENSE_FIELD, RAG_DEFAULT_FIELD_WEIGHTS, RAG_EVIDENCE_FIELD_QUOTAS, RAG_STANDARD_FIELDS
from rs_core.agent.rag.schema import RagEvidence
from rs_core.agent.rag.vector_index import LOCAL_VECTOR_METHOD, TextEmbeddingBackend, load_local_vector_index
...
class HybridCandidateRetriever:
    ...
    lexical_backend: CandidateEvidenceRetriever | None = None
    lexical_backend_name: str = "sqlite_bm25"
    legacy_sqlite_hashed_vector_fallback_enabled: bool = True
    ...
    def retrieve(
        ...
        bm25_evidence = []
        legacy_index_path = Path(self.index_path) if self.index_path else None
        has_bm25_index = bool(legacy_index_path and legacy_index_path.exists())
        has_vector_fallback = self._has_configured_vector_fallback()
        has_lexical_backend = self.lexical_backend is not None
        if self.lexical_backend is not None:
            try:
                bm25_evidence = self.lexical_backend.retrieve(query, candidate_ids, max_evidence_per_item)
            except Exception:
                if has_bm25_index and legacy_index_path is not None:
                    bm25_evidence = SQLiteBM25CandidateRetriever(legacy_index_path).retrieve(query, candidate_ids, max_evidence_per_item)
            ...
            try:
                ...
                if not has_vector_fallback:
                    raise
        elif not has_vector_fallback and not has_lexical_backend:
            return []
    ...
    def _has_configured_vector_fallback(self) -> bool:
        legacy_index_path = Path(self.index_path) if self.index_path else None
        manifest_vector_index_path = _manifest_vector_index_path(legacy_index_path) if legacy_index_path else None
        return self.vector_backend is not None or self.vector_index_path is not None or manifest_vector_index_path is not None
    ...
    def _vector_evidence(self, query: str, candidate_ids: list[str], max_evidence_per_item: int) -> list[RagEvidence]:
        ...
        legacy_index_path = Path(self.index_path) if self.index_path else None
        vector_index_path = self.vector_index_path or (_manifest_vector_index_path(legacy_index_path) if legacy_index_path else None)
        ...
        if not self.legacy_sqlite_hashed_vector_fallback_enabled or legacy_index_path is None or not legacy_index_path.exists():
            return []
        query_vector = text_to_hashed_vector(query, self.vector_dim)
        if not query_vector:
            return []

        chunks = _candidate_chunks(legacy_index_path, candidate_ids)
