...
def main() -> None:
    args = parse_args()
    ensure_ratio_args(args.valid_ratio, args.test_ratio)
    ensure_positive_args(
        args.min_user_interactions,
        args.min_item_interactions,
        args.sequence_max_len,
        args.small_data_all_train_threshold,
    )
    ...
    sqlite_path = output_dir / "recall_clean.sqlite"
    ...
    try:
        ...
        interaction_outputs = write_canonical_interactions(
            connection,
            output_dir,
            args.positive_rating_threshold,
            split_plan,
        )
        item_outputs = write_canonical_items(connection, output_dir)
        ...
        manifest_output = write_manifest(output_dir, manifest_payload)
        stats_output = write_stats(output_dir, stats_payload)
        print(f"Canonical interactions written to: {interaction_outputs['canonical_interactions_path']}")
        print(f"Canonical items written to: {item_outputs['canonical_items_path']}")
        print(f"User sequences written to: {sequence_outputs['user_sequences_path']}")
        print(f"Train user sequences written to: {sequence_outputs['train_user_sequences_path']}")
