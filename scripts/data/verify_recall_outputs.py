...
def run_checks(clean_stats: dict[str, Any], view_stats: dict[str, Any], require_strong_itemcf: bool) -> tuple[bool, list[str]]:
    lines: list[str] = []
    failures: list[str] = []
    ...
    checks = [
        ...
        (
            "weak_itemcf",
            weak_itemcf.get("rows_written", 0) > 0 and weak_itemcf.get("unique_pair_count", 0) > 0 and output_exists(weak_itemcf.get("output_path", "")),
            f"rows_written={weak_itemcf.get('rows_written', 0)}, unique_pair_count={weak_itemcf.get('unique_pair_count', 0)}",
            True,
        ),
        (
            "category_recall",
            view_category.get("category_rows_written", 0) > 0 and output_exists(view_category.get("category_items_path", "")),
            f"category_rows_written={view_category.get('category_rows_written', 0)}",
            True,
        ),
        (
            "strong_itemcf",
            strong_itemcf.get("rows_written", 0) > 0 and strong_itemcf.get("unique_pair_count", 0) > 0 and output_exists(strong_itemcf.get("output_path", "")),
...
def main() -> None:
    ...
    ok, lines = run_checks(clean_stats, view_stats, args.require_strong_itemcf)
    for line in lines:
        print(line)
