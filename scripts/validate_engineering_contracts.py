...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        dest="scripts",
        help="Script path to validate. Defaults to tracked scripts/*.py.",
    )
    parser.add_argument(
        "--test",
        action="append",
...
def main() -> None:
    ...
    script_paths = args.scripts or _workspace_paths(root, "scripts/*.py")
...
def _workspace_paths(root: Path, pattern: str) -> list[str]:
    return [path.relative_to(root).as_posix() for path in sorted(root.glob(pattern)) if path.is_file()]
