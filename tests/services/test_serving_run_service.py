...
def test_run_service_declares_compatibility_demo_boundary(capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, object] = {}
    ...
    assert run_service.COMPATIBILITY_DEMO_NOTE in output
    assert "canonical online HTTP entrypoint is rs_core.serving.api.online_app" in output
    assert captured["target"] == "rs_core.serving.api.app:app"
