...
from scripts.serving.import_candidate_store_to_postgres import _insert_sql, import_candidate_store_to_postgres
...
def test_importer_dry_run_classifies_pool500_usercf_rows(tmp_path: Path) -> None:
    ...
    result = import_candidate_store_to_postgres(inputs=[path], limit_rows=10, artifact_id="artifact-1")
...
def test_importer_dry_run_classifies_item_neighbor_rows(tmp_path: Path) -> None:
    path = tmp_path / "item_neighbors.jsonl"
    path.write_text(json.dumps({"src_item": "seed", "dst_item": "next", "source": "itemcf", "score": 0.9, "category": "books"}) + "\n", encoding="utf-8")

    result = import_candidate_store_to_postgres(inputs=[path], limit_rows=10)

    report = result["reports"][0]
    assert report["schema"] == "item_neighbors"
    assert report["status"] == "supported"
...
def test_importer_write_skips_unsupported_without_runner(tmp_path: Path) -> None:
    ...
    result = import_candidate_store_to_postgres(inputs=[path], write=True, runner=runner)
...
def test_importer_write_generates_psql_stdin_sql_with_on_conflict(tmp_path: Path) -> None:
    path = tmp_path / "usercf_candidates.jsonl"
    ...
    result = import_candidate_store_to_postgres(inputs=[path], write=True, runner=runner, batch_size=1, artifact_id="artifact-2")
    ...
    assert "ON CONFLICT (source, user_id, parent_asin) DO UPDATE" in sql


def test_item_neighbor_insert_sql_is_idempotent() -> None:
    sql = _insert_sql(
        "item_neighbors",
    ...
    assert "ON CONFLICT (source, src_item_id, dst_item_id) DO UPDATE" in sql
...
def test_importer_dedupes_duplicate_usercf_keys_before_write(tmp_path: Path) -> None:
    ...
    result = import_candidate_store_to_postgres(inputs=[path], write=True, runner=runner, batch_size=50)
...
def test_importer_treats_user_id_rows_as_usercf_even_with_seed_context(tmp_path: Path) -> None:
    path = tmp_path / "ambiguous.jsonl"
    path.write_text(json.dumps({"user_id": "u1", "src_item": "seed", "item_id": "i1", "score": 1.0}) + "\n", encoding="utf-8")

    result = import_candidate_store_to_postgres(inputs=[path], limit_rows=10)

    assert result["reports"][0]["schema"] == "usercf_candidates"


def test_importer_skips_non_finite_scores_and_rejects_write(tmp_path: Path) -> None:
    path = tmp_path / "nan_score.jsonl"
    path.write_text('{"user_id":"u1","item_id":"i1","score":NaN}\n', encoding="utf-8")
    calls: list[tuple[list[str], str]] = []
    ...
    assert calls == []
    assert result["reports"][0]["unsupported_rows"] == 1
    assert result["reports"][0]["write_status"] == "rejected_partial_artifact"
