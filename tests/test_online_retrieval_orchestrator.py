...
from rs_core.recsys.online_retrieval.orchestrator import CandidateRetrievalOrchestrator
from rs_core.recsys.types import RecallCandidate
from rs_core.workflow.online_recommendation import _has_online_retrieval_config
...
def test_real_serving_configs_use_method_level_candidate_store_providers(monkeypatch) -> None:
    ...
    for path in [Path("configs/serving/online_service.yaml"), Path("configs/serving/online_service.local_qdrant.yaml")]:
        ...
        assert "postgres_item_neighbors" not in provider_names
        assert "postgres_usercf" not in provider_names
        assert "postgres_category" not in provider_names
        assert "postgres_popular" not in provider_names
        for expected in [
