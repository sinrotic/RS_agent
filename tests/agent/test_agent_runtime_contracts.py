...
from rs_core.agent.runtime_core.contracts import (
    FAILURE_CONTRACTS,
...
from rs_core.agent.runtime import RUNTIME_TRACE_STEP_ORDER


PROJECT_ROOT = Path(__file__).resolve().parents[2]
GENERIC_RUNTIME_ROOT = PROJECT_ROOT / "rs_core" / "agent" / "runtime_core"
DOMAIN_IMPORT_PREFIXES = (
    "rs_core.agent.adapters",
    "rs_core.agent.clients",
    "rs_core.agent.context",
    "rs_core.agent.decision",
    "rs_core.agent.dialogue",
    "rs_core.agent.engine",
    "rs_core.agent.explanation",
    "rs_core.agent.feedback",
    "rs_core.agent.memory",
    "rs_core.agent.planner",
    "rs_core.agent.rag",
    "rs_core.agent.runtime",
    "rs_core.agent.tools",
    "rs_core.workflow",
    "rs_core.recsys",
)
...
def test_generic_core_does_not_import_domain_modules() -> None:
    violations: list[str] = []
    for path in GENERIC_RUNTIME_ROOT.glob("*.py"):
        tree = ast.parse(path.read_text(encoding="utf-8"), filename=str(path))
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if _matches_domain_import(alias.name):
                        violations.append(f"{path.relative_to(PROJECT_ROOT)} imports {alias.name}")
            elif isinstance(node, ast.ImportFrom) and node.module:
                if _matches_domain_import(node.module):
                    violations.append(f"{path.relative_to(PROJECT_ROOT)} imports {node.module}")

    assert violations == []


def _matches_domain_import(module: str) -> bool:
    return any(module == prefix or module.startswith(f"{prefix}.") for prefix in DOMAIN_IMPORT_PREFIXES)
