...
from rs_core.agent.rag import RAG_PARENT_PROFILE_FIELD, build_sqlite_bm25_index
from rs_core.common.recsys_types import AgentDecision
from rs_core.agent.context import build_context_bundle
from rs_core.agent.reward import build_reward_evidence
from rs_core.agent.runtime import RUNTIME_TRACE_STEP_ORDER, AgentRuntime
from rs_core.agent.contracts.schema import INTENT_RECOMMEND_REQUEST, AgentSession, AgentTurn, FeedbackConstraints
from rs_core.agent.tools import AgentToolCall
...
from rs_core.online.runtime.pool500 import OnlinePool500Recommender, _rank_return_top_k
...
def test_agent_orchestration_facade_rag_agent_shadow_is_internal_only_and_non_mutating():
    ...
    turn.rag_context = {
        ...
        "metadata": {
            "retriever": "hybrid_milvus_small2big",
    ...
    assert rag["retriever"] == "hybrid_milvus_small2big"
...
def test_runtime_boundary_source_restrictions():
    runtime_source = Path("rs_core/agent/runtime/__init__.py").read_text(encoding="utf-8")
