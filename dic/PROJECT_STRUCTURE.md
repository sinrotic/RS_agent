...
│  ├─ recsys/ serving/ workflow/                # 推荐在线服务逐步收敛到 rs_core/online/
│  ├─ rsagent/ recsys/rag/ semantic_description/ # Agent/RAG 逐步收敛到 rs_core/agent/；agent_runtime 已迁入 agent/runtime_core 与 agent/adapters 后删除
...
- `rs_core/data/`（其中 `rs_core/data/pipelines/` 是 dataproc 实现目录，`rs_core/data/features/` 是 feature 目标目录，`rs_core/data/artifacts/` 是 artifact 实现目录）
...
- `rs_core/online/recall/` 已承接通用 recall source canonical / fallback merge 工具；`rs_core/recsys/` 中排序、候选生成等其余存量逻辑继续按白名单渐进收束
