...
- 建立轻量检索入口，优先复用当前 semantic title / metadata / display card 信息，后续再评估 FAISS 或本地向量索引
- RAG 只为 Agent 提供上下文和证据，不替代召回、排序，也不绕过候选池边界直接生成商品
...
- 增加 RAG 相关观测：解释是否引用真实商品信息、推荐是否仍在候选池内、是否减少不存在商品或无根据卖点
- 重点保留可讲清楚的案例，而不是只堆指标
...
对应入口已沉淀在 `dic/phases/phase_1_5/PHASE_1_5_DEMO_SUMMARY.md`。
...
产物：`outputs/agent/canonical/agent_feedback_demo_canonical/`
...
产物：`outputs/agent/canonical/agent_conversation_demo_canonical/`
...
`configs/demo/hybrid_demo/hybrid_demo_small.yaml` 仍然保留，作为早期 smoke / 小样本验证入口。

### 当前推荐入口

下面这些配置是当前更推荐的观察入口：

- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000_no_injection.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000_lopo.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000_semantic_title.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000_lopo_semantic_title.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000_semantic_title_item_feature.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_1000_lopo_semantic_title_item_feature.yaml`

双塔向量召回旁路观察入口：

- `configs/demo/hybrid_demo/hybrid_demo_electronics_10000_semantic_title_two_tower_dssm.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_10000_lopo_semantic_title_two_tower_dssm.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_10000_semantic_title_two_tower_youtube_dnn.yaml`
- `configs/demo/hybrid_demo/hybrid_demo_electronics_10000_lopo_semantic_title_two_tower_youtube_dnn.yaml`
