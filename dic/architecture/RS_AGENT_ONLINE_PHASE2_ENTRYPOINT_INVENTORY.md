...
- source canonicalization / merge 工具层已物理迁入 `rs_core/online/recall`；旧 `rs_core/recsys/recall` active package 已删除，`scripts/recall/*` 仍是 recall artifact/index 构建入口，不是 online runtime 入口。
