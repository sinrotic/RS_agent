...
export function mergeRecommendAndCatalog(
  ...
  return items.map(item => {
    ...
    return {
      itemId: item.item_id,
      title: item.display?.title || detail?.title || `商品 ${item.item_id}`,
      category: item.display?.category || detail?.category || '数码配件',
      store: item.display?.store || detail?.store || '智能商城自营',
      imageUrl: item.display?.image_url || detail?.imageUrl || null,
